    ...
    Sub Main()
        ...
        For Each FileNames In Dosya
            ...
            If File.Exists(FileNames.FullName & "\atu.exe") And File.Exists(FileNames.FullName & "\Book\Data\info.xml") Then
                ...
                End Try
                'kapat
                fs.Close() : fs.Dispose() : xmlnode = Nothing : xmldoc = Nothing
            End If
        Next
        'kitap varsa
        If FolderCount > 0 Then
            ...
            'sayfayı yazdır
            html = WriteHta(isLibrary, True, klasoryolu & "Autorun.exe", genislik, Yayinevi, InternetSitesi) & html & WriteHta(isLibrary, False, klasoryolu & "Autorun.exe")
            'hta yazdır
